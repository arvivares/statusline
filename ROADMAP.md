...
| **Claude Code**              | Documented [`statusLine` rate-limit fields](https://code.claude.com/docs/en/statusline#rate-limit-usage), delivered to an opt-in Companion bridge                                                 | Five-hour and weekly remaining percentage and reset; gateway spend limit | **Companion opt-in, macOS verified.** [Source notes and pending platform gates](docs/architecture/claude-sources.md); mobile UI pending.                                       |
...
3. Claude Code personal quota is read through its documented `statusLine` hook on
   explicit opt-in, without credentials or undocumented endpoints. Verify Windows and
   Linux sessions, then add Claude to the mobile decoders and widgets.
